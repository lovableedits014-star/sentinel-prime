...
const ORIGEM_INFO: Record<Origem, { label: string; icon: any; desc: string }> = {
  gabinete_atendidos: {
    label: "Pessoas atendidas pelo gabinete",
    icon: Database,
    desc: "Importe o historico de atendimento. O resultado desta acao fica separado das demais filas.",
  },
  csv: {
    label: "Lista externa (Excel / CSV)",
    icon: FileSpreadsheet,
    desc: "Suba uma planilha .xlsx, .xls ou .csv. Mapeie as colunas e importe. Se preferir, ainda dá para colar como texto.",
  },
  estrutura: {
    label: "Estrutura eleitoral",
    icon: Users2,
    desc: "Coordenadores, líderes e cabos cadastrados na sua estrutura (com telefone).",
  },
  indicados_eleicao: {
    label: "Indicados (eleição)",
    icon: Vote,
    desc: "Pessoas indicadas pela estrutura na eleição atual.",
  },
  contratados: {
    label: "Contratados / liderados",
    icon: Database,
    desc: "Pessoas contratadas que aceitaram o termo (líderes e liderados).",
  },
  indicados_contratados: {
    label: "Indicados dos contratados",
    icon: Megaphone,
    desc: "Quem cada contratado indicou organicamente.",
  },
};
...
const normalizeDates = (value: string) =>
  String(value || "")
    .split(/[,;|]/)
    .map((part) => {
...
export default function NovaFilaWizard({ open, onOpenChange, clientId, onCreated }: Props) {
  ...
  const [colRegiao, setColRegiao] = useState(NONE);
  const [colQtdAtend, setColQtdAtend] = useState(NONE);
  const [colDatasAtend, setColDatasAtend] = useState(NONE);
  const [colAreasAtend, setColAreasAtend] = useState(NONE);
  ...
  useEffect(() => {
    ...
    setColRegiao(NONE);
    setColQtdAtend(NONE);
    setColDatasAtend(NONE);
    setColAreasAtend(NONE);
    setPasteMode(false);
    setPasteText("");
    setSkipGlobalDupes(true);
    setCidade("");
    setBairro("");
    setTipo("__all__");
    ...
    setIntro("");
    setPerguntas("");
    setTags("Não mora mais aqui\nNúmero errado\nPediu retorno");
    setWhatsappTemplate("");
    setModoDesignacao("compartilhada");
    setOpsMarcados(new Set());
    setNomeLista("");
  ...
  const onFile = async (file: File) => {
    ...
    try {
      ...
      setColRegiao(guessCol(hs, ["regiao", "região"]) || NONE);
      setColQtdAtend(guessCol(hs, ["quantidade de atend", "qtd atend", "quantidade"]) || NONE);
      setColDatasAtend(guessCol(hs, ["datas dos atend", "data do atend", "datas atend"]) || NONE);
      setColAreasAtend(
        guessCol(hs, ["areas de atendimento", "áreas de atendimento", "area de atendimento"]) ||
          NONE,
      );
  ...
  const csvRows = useMemo(() => {
    ...
    if (pasteMode) {
      const lines = pasteText
        ...
        .filter(Boolean);
      if (!lines.length)
        return [] as { nome: string; telefone: string; cidade?: string; bairro?: string }[];
      const header = lines[0]
        .toLowerCase()
        .split(/[,;\t]/)
        .map((h) => h.trim());
      const hasHeader =
        header.includes("nome") && (header.includes("telefone") || header.includes("celular"));
      const cN = hasHeader ? header.indexOf("nome") : 0;
      const cT = hasHeader
        ? header.indexOf("telefone") >= 0
          ? header.indexOf("telefone")
          : header.indexOf("celular")
        : 1;
      ...
      return (hasHeader ? lines.slice(1) : lines)
        .map((l) => {
          ...
          return {
            nome: p[cN] || "",
            telefone: onlyDigits(p[cT] || ""),
            cidade: cC >= 0 ? p[cC] : undefined,
            bairro: cB >= 0 ? p[cB] : undefined,
          };
        })
    ...
    for (const r of rows) {
      ...
      out.push({
        ...
        regiao: colRegiao !== NONE ? String(r[colRegiao] || "").trim() || undefined : undefined,
        quantidade_atendimentos:
          colQtdAtend !== NONE ? Number(r[colQtdAtend] || 0) || undefined : undefined,
        datas_atendimentos:
          colDatasAtend !== NONE
            ? normalizeDates(String(r[colDatasAtend] || "")) || undefined
            : undefined,
        areas_atendimento:
          colAreasAtend !== NONE ? String(r[colAreasAtend] || "").trim() || undefined : undefined,
      });
    }
    return out;
  }, [
  ...
  const canNext = () => {
    ...
    if (step === 3) {
      if (origem === "csv" || origem === "gabinete_atendidos") return csvRows.length > 0;
      return true;
    }
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        ...
        setPreviewing(false);
        if (error) {
          toast.error("Não foi possível calcular a prévia: " + error.message);
          return;
        }
        const r = (data as any) || {};
        setPreview({
  ...
  const finish = async () => {
    ...
    const opUnicoParaCriar =
      modoDesignacao === "compartilhada" && marcados.length === 1 ? marcados[0] : null;
    ...
    if ((origem === "csv" || origem === "gabinete_atendidos") && csvRows.length) {
      const importRpc =
        origem === "gabinete_atendidos"
          ? "tele_import_gabinete_atendidos"
      ...
      const { data: impData, error: impErr } = await supabase.rpc(
        importRpc as any,
        importArgs as any,
      );
      ...
      if (impErr) {
        setBusy(false);
        toast.error("Fila criada, mas falhou a importação: " + impErr.message);
        return;
      }
    ...
    if (campanhaId) {
      ...
      if (opsErr)
        toast.error("Fila criada, mas não foi possível salvar os operadores: " + opsErr.message);
    }
    ...
    if (opUnicoParaCriar && origem !== "csv" && origem !== "gabinete_atendidos" && campanhaId) {
      const { data: lista } = await supabase.rpc("tele_admin_listar_avulsos" as any, {
        _client_id: clientId,
        _campanha_id: campanhaId,
      });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[92vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            Nova fila de ligação — Passo {step} de {totalSteps}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="flex gap-1 mb-2">
          {Array.from({ length: totalSteps }).map((_, i) => (
            <div
              key={i}
              className={`h-1.5 flex-1 rounded ${i + 1 <= step ? "bg-primary" : "bg-muted"}`}
            />
          ))}
        </div>
        ...
        {step === 1 && (
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Dê um nome reconhecível. Os operadores vão escolher a fila pelo nome.
            </p>
            <div>
              ...
              <Input
                ...
                placeholder="Ex: Centro - 1ª rodada"
              />
            </div>
            <div>
              ...
              <Textarea
                ...
                placeholder="Objetivo, prazo, observações…"
              />
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-3">
            {origem === "csv" || origem === "gabinete_atendidos" ? (
              <>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-muted/50 p-4 rounded-lg border space-y-3">
                      <Label className="flex items-center gap-2">
                        <Database className="w-4 h-4 text-primary" /> Nome desta lista para
                        relatórios
                      </Label>
                      <Input
                      ...
                      <p className="text-[10px] text-muted-foreground italic">
                        Ao salvar com um nome, o sistema cria um agrupamento permanente para você
                        gerar relatórios individuais deste lote depois.
                      </p>
                    </div>
                  </div>
                )}
                ...
                {!pasteMode && fileName && (
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      {origem === "gabinete_atendidos" && (
                        <>
                          <div>
                            <Label>Regiao</Label>
                            <Select value={colRegiao} onValueChange={setColRegiao}>
                              <SelectTrigger>
                                <SelectValue placeholder="Nenhuma" />
                              </SelectTrigger>
                              <SelectContent>
                          ...
                          <div>
                            ...
                            <Select value={colQtdAtend} onValueChange={setColQtdAtend}>
                              <SelectTrigger>
                                <SelectValue placeholder="Nenhuma" />
                              </SelectTrigger>
                              <SelectContent>
                          ...
                          <div>
                            <Label>Datas dos atendimentos</Label>
                            <Select value={colDatasAtend} onValueChange={setColDatasAtend}>
                              <SelectTrigger>
                                <SelectValue placeholder="Nenhuma" />
                              </SelectTrigger>
                              <SelectContent>
                          ...
                          <div>
                            <Label>Areas de atendimento</Label>
                            <Select value={colAreasAtend} onValueChange={setColAreasAtend}>
                              <SelectTrigger>
                                <SelectValue placeholder="Nenhuma" />
                              </SelectTrigger>
                              <SelectContent>
                    ...
                    <Card
                      className={`p-3 flex items-center gap-3 ${invalidCount > 0 ? "border-amber-400" : "border-emerald-400"}`}
                    >
                      {invalidCount > 0 ? (
                      ...
                      <div className="text-sm">
                        ...
                        {invalidCount > 0 && (
                          <span className="text-muted-foreground">
                            {" "}
                            · {invalidCount} ignorado(s) (sem nome ou telefone)
                          </span>
                        )}
                      </div>
                    </Card>
                ...
                {pasteMode && (
                  <>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {csvRows.length > 0
                        ? `${csvRows.length} linha(s) válida(s) detectadas.`
                        : "Cole linhas com nome e telefone."}
                    </p>
                  </>
                )}
              ...
              <>
                <p className="text-sm text-muted-foreground">
                  Filtre quem entra na fila. Deixe em branco para incluir todos.
                </p>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <Label>Cidade</Label>
                ...
                <div className="space-y-2 pt-2 border-t">
                  <label className="flex items-center gap-2 text-sm">
                    <Checkbox
                      ...
                      onCheckedChange={(v) => setApenasPendentes(!!v)}
                    />
        ...
        {step === 4 && (
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Script e tags ajudam o operador na hora da ligação. Tudo opcional.
            </p>
            <div>
              ...
              <Textarea
                ...
                placeholder="Bom dia, aqui é {operador} falando em nome do candidato…"
              />
            </div>
            <div>
              ...
              <Textarea
                ...
                placeholder={
                  "Você costuma votar nas eleições municipais?\nO que mais te preocupa hoje no bairro?"
                }
              />
            </div>
            <div>
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder="Olá {{nome}}, aqui é da campanha…"
              />
              <p className="text-[11px] text-muted-foreground mt-1">
                Aparece pré-preenchida quando o operador clicar em "Abrir WhatsApp". Use{" "}
                {"{{nome}}"} para inserir o nome do contato.
              </p>
            </div>
          </div>
        )}
        ...
        {step === 5 && (
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Marque quais operadores podem trabalhar esta fila. Quem não estiver marcado não recebe
              nenhum contato dela.
            </p>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    variant="ghost"
                    size="sm"
                    onClick={() => setOpsMarcados(new Set(opsAtivos.map((o) => o.id)))}
                  >
                    Marcar todos
                  </Button>
                  <Button
                    ...
                    variant="ghost"
                    size="sm"
                    onClick={() => setOpsMarcados(new Set())}
                  >
                    Limpar
                  </Button>
                </div>
              ...
              <div className="grid gap-1.5 sm:grid-cols-2">
                {opsAtivos.map((o) => {
                  ...
                  return (
                    <label
                      key={o.id}
                      className={`flex items-center gap-2 border rounded-md px-2 py-1.5 text-sm cursor-pointer ${checked ? "bg-primary/10 border-primary" : "hover:bg-muted/50"}`}
                    >
                      <Checkbox
                        checked={checked}
                        onCheckedChange={(v) => {
                ...
                {opsAtivos.length === 0 && (
                  <p className="text-xs text-amber-600">Nenhum operador ativo cadastrado.</p>
                )}
              </div>
              <p className="text-[11px] text-muted-foreground">
                {opsMarcados.size} operador(es) marcado(s)
              </p>
            </div>

            <p className="text-sm text-muted-foreground">
              Como distribuir os contatos entre os marcados?
            </p>
            ...
            >
              ...
              <p className="text-xs text-muted-foreground">
                Os operadores marcados puxam contatos do mesmo bolo. O sistema impede que dois
                peguem o mesmo contato. Se só um estiver marcado, a fila inteira fica com ele.
              </p>
            </button>
            ...
            >
              ...
              <p className="text-xs text-muted-foreground">
                Distribuição round-robin: cada operador recebe uma fatia fixa (exige 2 ou mais
                marcados).
              </p>
            </button>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="rounded-lg border p-3 space-y-2 text-sm">
              ...
              <div className="flex justify-between gap-2">
                ...
                <Badge variant="outline">{ORIGEM_INFO[origem].label}</Badge>
              </div>
              {origem === "csv" || origem === "gabinete_atendidos" ? (
                <div className="flex justify-between gap-2">
                  <span className="text-muted-foreground">Contatos na lista</span>
                ...
                <>
                  ...
                  <div className="flex justify-between gap-2">
                    <span className="text-muted-foreground">Apenas não ligados</span>
                    <span>{apenasPendentes ? "Sim" : "Não"}</span>
                  </div>
                </>
              )}
              <div className="flex justify-between gap-2">
                ...
                <span className="text-right">
                  {modoDesignacao === "compartilhada" &&
                    `Compartilhada entre ${opsMarcados.size} operador(es)`}
            ...
            <div className="rounded-lg border p-3 text-sm">
              {previewing ? (
                <span className="flex items-center gap-2 text-muted-foreground">
                  <Loader2 className="w-4 h-4 animate-spin" /> Calculando quantos contatos vão
                  entrar…
                </span>
              ) : preview ? (
                <div className="space-y-1">
                  <p className="font-medium flex items-center gap-2">
                    {previewEntrarao > 0 ? (
                      <>
                        <CheckCircle2 className="w-4 h-4 text-emerald-600" /> {previewEntrarao}{" "}
                        contato(s) vão entrar nesta fila
                      </>
                    ) : (
                      <>
                        <AlertCircle className="w-4 h-4 text-amber-600" /> Nenhum contato atende a
                        esses filtros
                      </>
                    )}
                  </p>
                  ...
                  {previewEntrarao === 0 && (
                    <p className="text-xs text-amber-600">
                      Volte ao passo 3 e ajuste os filtros — a fila seria criada vazia.
                    </p>
                  )}
                </div>
              ) : (
            ...
            <p className="text-xs text-muted-foreground">
              Ao confirmar, a fila será criada e os contatos vinculados. Depois você pode usar
              "Adicionar contatos" para complementar a fila e "Gerenciar designações" para ajustar
              os operadores.
            </p>
          </div>
        )}

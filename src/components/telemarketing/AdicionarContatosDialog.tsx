...
const ORIGENS: { key: Origem; label: string; icon: any; desc: string }[] = [
  {
    ...
    desc: "Suba uma planilha com nome e telefone.",
  },
  {
    key: "estrutura",
    label: "Estrutura eleitoral",
    icon: Users2,
    desc: "Coordenadores, líderes e cabos com telefone.",
  },
  {
    key: "indicados_eleicao",
    label: "Indicados (eleição)",
    icon: Vote,
    desc: "Pessoas indicadas pela estrutura.",
  },
  {
    key: "contratados",
    label: "Contratados / liderados",
    icon: Database,
    desc: "Contratados cadastrados.",
  },
  {
    key: "indicados_contratados",
    label: "Indicados dos contratados",
    icon: Megaphone,
    desc: "Quem cada contratado indicou.",
  },
];
...
export default function AdicionarContatosDialog({
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        ...
        if (error) {
          setIndicadores([]);
          toast.error("Erro ao carregar responsáveis", { description: error.message });
          return;
        }
  ...
  }, [open, clientId, origem]);

  const indicadorLabel =
    origem === "contratados"
      ? "Todos os líderes"
      : origem === "indicados_contratados"
  ...
  const carregarPreview = async () => {
    ...
    setPreviewing(false);
    if (error) {
      toast.error("Erro na prévia: " + error.message);
      return;
    }
    const r = (data as any) || {};
    setPreview({
  ...
  const confirmar = async () => {
    ...
    try {
      ...
      if (origem === "csv") {
        ...
        if (adicionados > 0 && designar !== ALL) {
          ...
          const ids = ((lista as any[]) || [])
            .filter(
              (c: any) =>
                (!c.ligacao_status || c.ligacao_status === "pendente") && !c.assigned_operador_id,
            )
            .map((c: any) => c.id);
          if (ids.length) {
      ...
      if (adicionados === 0) {
        toast.warning("Nenhum contato novo foi adicionado", {
          description: "Todos já estavam nesta fila ou não atendem aos filtros.",
        });
      } else {
        ...
        <div className="space-y-4">
          ...
          {origem === "csv" ? (
            <div className="space-y-3">
              {!fileName ? (
                <Card
                  className="p-6 text-center border-dashed border-2 cursor-pointer hover:bg-muted/40"
                ...
                <div className="space-y-2">
                  <p className="text-xs text-muted-foreground">
                    {fileName} · {rows.length} linhas · {csvRows.length} válidas
                  </p>
                ...
                <p className="font-medium flex items-center gap-2">
                  {entrarao > 0 ? (
                    <>
                      ...
                      {origem === "csv" ? csvRows.length : entrarao} contato(s) serão adicionados
                    </>
                  ) : (
                    <>
                ...
                {origem !== "csv" && (
                  <p className="text-xs text-muted-foreground">
                    ...
                    {preview.ja_em_outra_fila > 0 &&
                      ` · ${preview.ja_em_outra_fila} em outra fila (${substituir ? "serão movidos" : "ignorados"})`}
                  </p>
                )}
              </div>
            ) : (

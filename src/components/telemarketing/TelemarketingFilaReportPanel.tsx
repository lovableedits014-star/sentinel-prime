...
import {
  CheckCircle2, FileDown, FileSpreadsheet, FilterX, Loader2, ListChecks,
...
const VOTE_LABEL: Record<string, string> = {
  sim: "Vota", nao: "Não vota", indeciso: "Indeciso", nao_quis_opinar: "Não quis opinar", sem_resposta: "Sem resposta",
};
const RESULT_LABEL: Record<string, string> = {
  atendeu: "Atendeu", nao_atendeu: "Não atendeu", reagendou: "Reagendou", invalido: "Inválido", pendente: "Pendente",
};
const VOTE_COLORS: Record<string, string> = {
  sim: "#22c55e", nao: "#ef4444", indeciso: "#f59e0b", nao_quis_opinar: "#8b5cf6", sem_resposta: "#94a3b8",
};
...
const topNames = (rows: FilaReportRow[], key: keyof FilaReportRow, limit = 10) => {
  ...
  return [...map.entries()].sort((a, b) => b[1] - a[1]).slice(0, limit).map(([nome, total]) => ({ nome, total }));
};
...
export default function TelemarketingFilaReportPanel({ clientId, campanhaId, campanhaNome }: Props) {
  ...
  const filtered = useMemo(() => rows.filter((r) => {
    ...
    if (q && !`${r.nome} ${r.telefone} ${r.indicador_nome || ""}`.toLocaleLowerCase("pt-BR").includes(q)) return false;
  ...
  const kpi = useMemo(() => {
    ...
    const invalidos = filtered.filter((r) => r.ligacao_status === "invalido" || r.status_telemarketing === "descartado").length;
    const tentativas = filtered.reduce((s, r) => s + (r.total_tentativas || 0), 0);
    return {
      total, trabalhados, pendentes: total - trabalhados, atendidos, tentativas,
      sim: count("sim"), nao: count("nao"), indeciso: count("indeciso"),
      naoQuisOpinar: count("nao_quis_opinar"), naoAtendeu, invalidos,
      cobertura: pct(trabalhados, total), taxaContato: pct(atendidos, trabalhados),
  ...
  const activeFilters = [search, operator, origin, city, neighborhood, result, vote, from, to].filter((v) => v && v !== ALL).length;
  const resetFilters = () => {
    setSearch(""); setOperator(ALL); setOrigin(ALL); setCity(ALL); setNeighborhood(ALL);
    setResult(ALL); setVote(ALL); setFrom(""); setTo("");
  };

  const detailRows = () => filtered.map((r) => ({
    Fila: clean(r.campanha_nome), Origem: r.origem, Contato: r.nome, Telefone: r.telefone,
    Cidade: clean(r.cidade), Bairro: clean(r.bairro), Indicador: clean(r.indicador_nome),
    ...
    Federal: clean(r.candidato_federal), Senador: clean(r.candidato_senador), Governador: clean(r.candidato_governador),
  ...
  const exportExcel = () => {
    ...
    XLSX.writeFile(wb, `telemarketing-${slug(campanhaNome)}-${new Date().toISOString().slice(0, 10)}.xlsx`);
  };
  ...
  const exportPDF = () => {
    ...
    doc.setFontSize(9);
    doc.text(`Gerado em ${new Date().toLocaleString("pt-BR")} | ${filtered.length} contato(s)${activeFilters ? " | filtros aplicados" : ""}`, 36, 55);
    autoTable(doc, {
      startY: 72,
      head: [["Contatos", "Trabalhados", "Pendentes", "Atendidos", "Vota", "Não vota", "Indeciso", "Não opinou", "Não atendeu", "Cobertura", "Conversão"]],
      body: [[kpi.total, kpi.trabalhados, kpi.pendentes, kpi.atendidos, kpi.sim, kpi.nao, kpi.indeciso, kpi.naoQuisOpinar, kpi.naoAtendeu, `${kpi.cobertura}%`, `${kpi.conversao}%`]],
    ...
    autoTable(doc, {
      head: [["Origem", "Contato", "Telefone", "Bairro", "Resultado", "Voto", "Federal", "Senador", "Governador", "Operador"]],
      body: filtered.map((r) => [
        r.origem, r.nome, r.telefone, clean(r.bairro),
        RESULT_LABEL[normalizedResult(r)] || normalizedResult(r),
        VOTE_LABEL[r.vota_candidato || "sem_resposta"],
        clean(r.candidato_federal), clean(r.candidato_senador), clean(r.candidato_governador), clean(r.operador_nome),
      ]),
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Vale para qualquer lista de ligação (planilha, estrutura, indicados, contratados, avulsos). Use o seletor de fila acima para medir cada fila separadamente.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="outline" size="sm" onClick={exportExcel} disabled={!filtered.length}><FileSpreadsheet className="size-4" /> Excel</Button>
            <Button variant="outline" size="sm" onClick={exportPDF} disabled={!filtered.length}><FileDown className="size-4" /> PDF</Button>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-5">
        <div className="grid gap-2 md:grid-cols-3 xl:grid-cols-4">
          <div className="relative">
            ...
            <Input className="pl-9" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Nome, telefone ou indicador" />
          </div>
          <Select value={origin} onValueChange={setOrigin}>
            <SelectTrigger><SelectValue placeholder="Origem" /></SelectTrigger>
            <SelectContent><SelectItem value={ALL}>Todas as origens</SelectItem>{options.origins.map((v) => <SelectItem key={v} value={v}>{v}</SelectItem>)}</SelectContent>
          </Select>
          <Select value={operator} onValueChange={setOperator}>
            <SelectTrigger><SelectValue placeholder="Operador" /></SelectTrigger>
            <SelectContent><SelectItem value={ALL}>Todos os operadores</SelectItem>{options.operators.map((v) => <SelectItem key={v} value={v}>{v}</SelectItem>)}</SelectContent>
          </Select>
          <Select value={result} onValueChange={setResult}>
            <SelectTrigger><SelectValue placeholder="Resultado" /></SelectTrigger>
            <SelectContent>
              <SelectItem value={ALL}>Todos os resultados</SelectItem>
              {Object.entries(RESULT_LABEL).map(([k, v]) => <SelectItem key={k} value={k}>{v}</SelectItem>)}
            </SelectContent>
          </Select>
          <Select value={vote} onValueChange={setVote}>
            <SelectTrigger><SelectValue placeholder="Voto" /></SelectTrigger>
            <SelectContent>
              <SelectItem value={ALL}>Todos os votos</SelectItem>
              {Object.entries(VOTE_LABEL).map(([k, v]) => <SelectItem key={k} value={k}>{v}</SelectItem>)}
            </SelectContent>
          </Select>
          <Select value={city} onValueChange={setCity}>
            <SelectTrigger><SelectValue placeholder="Cidade" /></SelectTrigger>
            <SelectContent><SelectItem value={ALL}>Todas as cidades</SelectItem>{options.cities.map((v) => <SelectItem key={v} value={v}>{v}</SelectItem>)}</SelectContent>
          </Select>
          <Select value={neighborhood} onValueChange={setNeighborhood}>
            <SelectTrigger><SelectValue placeholder="Bairro" /></SelectTrigger>
            <SelectContent><SelectItem value={ALL}>Todos os bairros</SelectItem>{options.neighborhoods.map((v) => <SelectItem key={v} value={v}>{v}</SelectItem>)}</SelectContent>
          </Select>
        ...
        {activeFilters > 0 && (
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" size="sm" onClick={resetFilters}><FilterX className="size-4" /> Limpar filtros</Button>
          </div>
        )}

        {loading && <div className="flex items-center gap-2 text-sm text-muted-foreground"><Loader2 className="size-4 animate-spin" /> Carregando contatos…</div>}

        {!loading && rows.length === 0 && (
          <p className="text-sm text-muted-foreground">Nenhum contato vinculado a esta fila ainda.</p>
        )}
          ...
          <>
                ...
                <ResponsiveContainer width="100%" height={230}>
                  <PieChart>
                    <Pie data={voteChart} dataKey="value" nameKey="name" outerRadius={80} label>
                      {voteChart.map((d) => <Cell key={d.key} fill={VOTE_COLORS[d.key]} />)}
                    </Pie>
                    <Tooltip />
                    <Legend />
                  </PieChart>
              ...
              <div className="rounded-lg border p-3">
                ...
                <ResponsiveContainer width="100%" height={230}>
                  <BarChart data={resultChart}>
                    ...
                    <Tooltip />
                    <Bar dataKey="total" name="Contatos" fill="hsl(var(--primary))" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
              <div className="rounded-lg border p-3">
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              <div className="rounded-lg border p-3">
                ...
                <Table>
                  <TableHeader><TableRow><TableHead>Bairro</TableHead><TableHead className="text-right">Contatos</TableHead><TableHead className="text-right">Vota sim</TableHead></TableRow></TableHeader>
                  <TableBody>
                    {bairroRanking.map((b) => (
                      <TableRow key={b.nome}><TableCell>{b.nome}</TableCell><TableCell className="text-right">{b.total}</TableCell><TableCell className="text-right font-semibold">{b.sim}</TableCell></TableRow>
                    ))}
                  </TableBody>
                </Table>
              </div>
              <div className="space-y-3">
                {[["Deputado federal", federal], ["Senador", senador], ["Governador", governador], ["Vota em (quando não vota)", alternativos]].map(([title, list]) => (
                    ...
                    {(list as { nome: string; total: number }[]).length === 0
                      ...
                      : <div className="flex flex-wrap gap-1">
                          {(list as { nome: string; total: number }[]).map((c) => (
                            <Badge key={c.nome} variant="outline" className="text-[11px]">{c.nome} · {c.total}</Badge>
                          ))}
                        </div>}
                  </div>
                ))}
                ...
                <AccordionContent>
                  <div className="max-h-[520px] overflow-auto">
                    <Table>
                      <TableHeader>
                        <TableRow>
                          <TableHead>Contato</TableHead><TableHead>Telefone</TableHead><TableHead>Origem</TableHead>
                          <TableHead>Bairro</TableHead><TableHead>Resultado</TableHead><TableHead>Voto</TableHead>
                          <TableHead>Federal</TableHead><TableHead>Operador</TableHead><TableHead>Última ligação</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {filtered.map((r) => (
                          <TableRow key={`${r.tabela}-${r.contato_id}`}>
                            ...
                            <TableCell className="text-xs">{RESULT_LABEL[normalizedResult(r)] || normalizedResult(r)}</TableCell>
                            <TableCell className="text-xs">{VOTE_LABEL[r.vota_candidato || "sem_resposta"]}</TableCell>
                            ...
                            <TableCell className="text-xs">{r.ligacao_em ? new Date(r.ligacao_em).toLocaleString("pt-BR") : "—"}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>

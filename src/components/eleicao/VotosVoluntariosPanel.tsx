...
export default function VotosVoluntariosPanel({
  ...
  return (
    <Card className={bloqueado ? "opacity-60" : undefined}>
      <CardHeader className="pb-2">
        <CardTitle className={`text-base flex items-center gap-2 ${bloqueado ? "text-muted-foreground" : ""}`}>
          ...
          <Button size="icon" variant="ghost" className="h-7 w-7 ml-auto" onClick={load} title="Atualizar" disabled={bloqueado}>
        ...
        {bloqueado ? (
          <div className="rounded-md border-2 border-dashed border-muted-foreground/30 bg-muted/40 p-5 text-center space-y-2">
            ...
            <p className="text-xs text-muted-foreground/80 max-w-md mx-auto">
              O administrador vai liberar esta área no momento certo. Por enquanto, você não precisa fazer nada aqui.
            </p>
          </div>
        ) : (
          <>

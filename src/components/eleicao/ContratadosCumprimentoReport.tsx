      ...
      <Card className="overflow-hidden border-indigo-200 bg-gradient-to-br from-indigo-950 via-indigo-900 to-violet-800 text-white">
        <CardContent className="flex flex-col gap-4 p-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h3 className="mt-1 text-2xl font-bold">{isMissions ? "Ranking pela taxa real de cumprimento" : "Ranking por quantidade de votos"}</h3>
            <p className="mt-1 max-w-3xl text-xs text-indigo-200">{isMissions ? "A posição considera somente a taxa real: missões concluídas divididas pelas missões atribuídas." : "A posição considera somente o total acumulado de votos confirmados. Indicados e conversão são apenas informativos."}</p>
          </div>
        ...
        <Card className="border-sky-200 bg-sky-50/60 dark:bg-sky-950/20">
          ...
          <CardContent className="text-sm">
            <div className="rounded-lg border bg-background p-3">
              <strong>Taxa real = missões concluídas ÷ missões atribuídas</strong>
      ...
      {!!podiumRows.length && <div className="grid gap-3 md:grid-cols-3">{podiumRows.map((row, index) => (
        <Card key={row.id} className={index === 0 ? "border-amber-300 bg-gradient-to-b from-amber-50 to-background shadow-md" : "bg-card"}>
          <CardContent className="p-5">
            ...
            <p className="mt-4 text-4xl font-black tabular-nums">{isMissions ? `${row.missionRate.toFixed(1)}%` : row.confirmed}</p><p className="text-xs text-muted-foreground">{isMissions ? "taxa real de cumprimento" : "votos confirmados"}</p>

...
export default function DailyEngagementOperations({ clientId }: { clientId: string }) {
  ...
  const todayCards = [["Interações hoje",t.eventos,Activity],["Pessoas identificadas",t.pessoas_identificadas,Users],["Confirmações registradas",t.confirmacoes_evento,CheckCircle2],["Líderes que concluíram hoje",t.lideres_concluiram,CheckCircle2],["Cliques hoje",t.cliques,MousePointerClick]] as const;
  ...
  return <div className="space-y-4">
    ...
    <section><h2 className="mb-2 text-sm font-semibold">Movimento de hoje</h2><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{todayCards.map(([label,value,Icon])=><Card key={label}><CardContent className="flex items-center gap-3 p-4"><Icon className="h-5 w-5 text-primary"/><div><p className="text-xs text-muted-foreground">{label}</p><p className="text-2xl font-bold">{value}</p></div></CardContent></Card>)}</div></section>

...
import { exportCoordinatorMissionPdf, exportCoordinatorMissionSummaryPdf, exportStandaloneMissionPdf } from "@/lib/engagement-coordinator-pdf";
...
type CoordinatorTeam = {
  ...
  coordenador_status: "cumpriu"|"abriu"|"nao_abriu"; coordenador_primeiro_acesso: string|null; coordenador_cumprido_em: string|null;
};
...
type TrackingAudit = { checkins_concluidos: number; concluidos_sem_vinculo: number; obrigacoes_concluidas_pendentes: number; telefones_duplicados_eleicao: number; auditado_em: string };
type StandaloneContract = { pessoa_id: string; nome: string; telefone: string | null; cargo: string | null; regiao: string | null; cidade: string | null; status: "cumpriu"|"abriu"|"nao_abriu"; primeiro_acesso_em: string|null; cumprido_em: string|null };
type ActivitySummary = {
  periodo: "hoje" | "todo"; inicio: string; fim: string; eventos_brutos: number; acoes_unicas: number;
  ...
  timeline: { chave: string; rotulo: string; pessoas: number; pessoas_clicaram: number; confirmacoes: number; eventos_brutos: number }[];
  updated_at: string;
};
...
export default function DailyEngagementOperations({ clientId }: { clientId: string }) {
  ...
  const activity = useQuery({
    queryKey: ["engagement-mission-activity-period", clientId, missionId, activityPeriod],
    queryFn: async () => {
      const { data, error } = await db.rpc("engagement_mission_activity_period", {
        p_client_id: clientId, p_mission_id: missionId, p_periodo: activityPeriod,
      });
      if (error) throw new Error(error.message);
  ...
  const chargeCertified = completionAudit.isSuccess && completionAudit.data.consistente
    && trackingAudit.isSuccess && trackingAudit.data.obrigacoes_concluidas_pendentes === 0;
  ...
  const exportStandalonePdf = async () => {
    if(!data?.mission||!standaloneContracts.data?.length)return;
    setBusy("pdf:standalone");
    try{await exportStandaloneMissionPdf({missionTitle:data.mission.title,publishedAt:data.mission.published_at,rows:standaloneContracts.data});toast.success("Relatório dos avulsos gerado.");}
    catch(error){toast.error(error instanceof Error?error.message:"Erro ao gerar relatório dos avulsos");}
    finally{setBusy(null);}
  };
  ...
  const maxTimeline=Math.max(1,...timeline.map((h)=>n(h.pessoas)));
  const ageHours=Math.max(0,Math.floor((Date.now()-published.getTime())/3_600_000));
  const cumulativeCards = [
    ["Pessoas acompanhadas",c.obrigados,Users,"Todos os contratados ativos, todos os coordenadores ativos e demais pessoas do sistema que se identificaram nesta missão."],
    ["Concluíram",c.concluidos,CheckCircle2,"Pessoas acompanhadas que acessaram todos os links da missão."],
    ["Abriram, não concluíram",c.abriu_sem_concluir,Clock3,"Pessoas acompanhadas identificadas que ainda não acessaram todos os links."],
    ["Nunca abriram",c.nao_abriu,XCircle,"Pessoas acompanhadas sem nenhum acesso reconhecido nesta missão."],
    ["Adesão acumulada",`${n(c.taxa).toFixed(1)}%`,Activity,"Percentual de pessoas acompanhadas que concluíram a missão."],
  ] as const;
  const periodLabel=activityPeriod==="hoje"?"hoje":"desde a publicação";
  const todayCards = [
    ["Pessoas que entraram",t?.pessoas_identificadas??0,Users,`Pessoas únicas identificadas no link ${periodLabel}. Cada pessoa conta uma vez no período selecionado.`],
    ["Reconhecidas entre as que entraram",t?.pessoas_vinculadas??0,Users,`Das pessoas que entraram ${periodLabel}, quantas foram associadas a algum cadastro. Este número não é o total de contratados.`],
    ["Pessoas que clicaram",t?.pessoas_clicaram??0,MousePointerClick,`Pessoas únicas que abriram ao menos um link ${periodLabel}. Não representa curtidas do Instagram.`],
    ...
    ["Contratados que concluíram",t?.lideres_concluiram??0,CheckCircle2,`Conclusões ${periodLabel} conciliadas com os contratados obrigados do funil acima.`],
  ] as const;

...
import { Activity, CheckCircle2, Clock3, Info, MessageCircle, MousePointerClick, RefreshCw, Search, Users, XCircle } from "lucide-react";
...
export default function DailyEngagementOperations({ clientId }: { clientId: string }) {
  ...
  const cumulativeCards = [
    ["Líderes obrigados",c.obrigados,Users,"Pessoas da lista de líderes que precisam cumprir esta missão."],
    ["Concluíram",c.concluidos,CheckCircle2,"Líderes obrigados que acessaram todos os links da missão."],
    ["Abriram, não concluíram",c.abriu_sem_concluir,Clock3,"Líderes obrigados identificados que ainda não acessaram todos os links."],
    ["Nunca abriram",c.nao_abriu,XCircle,"Líderes obrigados sem nenhum acesso reconhecido nesta missão."],
    ["Adesão acumulada",`${n(c.taxa).toFixed(1)}%`,Activity,"Percentual dos líderes obrigados que concluíram: concluíram ÷ obrigados."],
  ] as const;
  const todayCards = [
    ...
    ["Vinculadas ao sistema",t?.pessoas_vinculadas??0,Users,"Pessoas identificadas cujo telefone foi associado a um cadastro existente no sistema."],
    ...
    ["Pessoas que confirmaram",t?.pessoas_confirmaram??0,CheckCircle2,"Todos os participantes que acessaram todos os links, sejam ou não líderes obrigados."],
    ["Líderes que concluíram",t?.lideres_concluiram??0,CheckCircle2,"Somente conclusões de hoje que foram conciliadas com os líderes obrigados do funil acima."],
  ] as const;

  return <div className="space-y-4">
    <Card><CardHeader><CardTitle className="text-base">Operação da missão</CardTitle><CardDescription>O funil mede somente os líderes obrigados. O movimento de hoje mostra todas as pessoas que usaram o link, separadamente.</CardDescription></CardHeader><CardContent className="flex flex-wrap items-end gap-3"><div className="min-w-[280px] flex-1 space-y-1"><p className="text-xs font-medium">Missão acompanhada</p><Select value={missionId} onValueChange={setMissionId}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent>{(missions.data??[]).map((m)=><SelectItem key={m.mission_id} value={m.mission_id}>{m.titulo}</SelectItem>)}</SelectContent></Select></div><div className="text-xs text-muted-foreground"><p>Publicada em {published.toLocaleString("pt-BR")}</p><p>No ar há {ageHours<24?`${ageHours}h`:`${Math.floor(ageHours/24)} dias`}</p></div><Button variant="outline" onClick={()=>{center.refetch();activity.refetch();}} disabled={center.isFetching||activity.isFetching}><RefreshCw className={`mr-1 h-4 w-4 ${center.isFetching||activity.isFetching?"animate-spin":""}`}/>Atualizar</Button></CardContent></Card>
    <section><h2 className="mb-1 text-sm font-semibold">Resultado acumulado do público obrigatório</h2><p className="mb-2 text-xs text-muted-foreground">Esses cinco indicadores usam o mesmo universo de {c.obrigados} líderes; por isso concluíram + abriram + nunca abriram sempre fecha o total.</p><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{cumulativeCards.map(([label,value,Icon,help])=><MetricCard key={label} label={label} value={value} Icon={Icon} help={help}/>)}</div></section>

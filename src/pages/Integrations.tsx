...
import { Facebook as FacebookIcon as Facebook as FacebookIcon, Brain, Save, AlertCircle, Zap, Check, Loader2, ShieldCheck, ShieldAlert, RefreshCw, Instagram as InstagramIcon as Instagram as InstagramIcon, MessageSquareText } from "lucide-react";
...
const REQUIRED_PERMISSIONS = [
  ...
  { name: 'pages_manage_posts', label: 'Publicar no Facebook as FacebookIcon', required: false, platform: 'facebook' },
  { name: 'instagram_content_publish', label: 'Publicar no Instagram as InstagramIcon', required: false, platform: 'instagram' },
];
...
const Integrations = () => {
  ...
  const fetchClientAndIntegrations = async () => {
    try {
  ...
  const handleRenewToken = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.expired) {
          setTokenStatus(prev => ({ ...prev, isExpired: true }));
        }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (metaData.accessToken && metaData.accessToken.trim() !== "") {
        ...
        updateData.meta_token_expires_at = null; // Will be checked on next renewal
      }
      ...
      if (llmData.provider === 'lovable') {
        ...
        if (llmData.apiKey && llmData.apiKey.trim() !== "") {
          updateData.llm_api_key = llmData.apiKey;
        }
      ...
      // Auto-renew token after saving a new one
      if (metaData.accessToken && metaData.accessToken.trim() !== "") {
        setTokenStatus(prev => ({ ...prev, isExpired: false, isExpiringSoon: false }));
        // Try to derive long-lived token in background
        handleRenewToken();
      }
    } catch (error: any) {
    ...
    <div className="p-8 space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-2 text-sm max-w-3xl">
          Configure a inteligência do seu painel. Conecte sua página do Facebook as FacebookIcon e Instagram as InstagramIcon para monitorar comentários, e escolha entre diferentes provedores de IA (Lovable, OpenAI, Anthropic, Gemini) para automatizar respostas e análises de sentimento.
        </p>
      </div>
      ...
      <Card className="border-warning/50 bg-warning/5">
        <CardContent className="flex items-start gap-3 pt-6">
          ...
          <div className="text-sm">
            <p className="font-medium text-warning mb-1">Segurança</p>
          ...
          {llmData.provider === 'lovable' ? (
            ...
            <>
              ...
              <Button 
                ...
                disabled={testingLLM || !llmData.apiKey}
                variant="outline"
                className="w-full"
              >
                {testingLLM ? (
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              id="meta-token"
              type="password"
              placeholder="Insira um novo token para atualizar"
            ...
            <div className={`flex items-start gap-3 p-4 rounded-lg border ${
              ...
              <div className="flex-1">
                ...
                <p className={`text-xs mt-1 ${
                  ...
                  {tokenStatus.isExpired 
                    ? 'A sincronização de comentários está parada. Gere um novo token no Meta for Developers ou tente renovar.'
                    : tokenStatus.isExpiringSoon && tokenStatus.expiresAt
                      ...
                      : tokenStatus.neverExpires
                        ? 'Token de página sem expiração. Sincronização funcionando normalmente.'
                        : ''}
                </p>
                {(tokenStatus.isExpired || tokenStatus.isExpiringSoon) && (
                  <Button 
                    size="sm" 
                    variant={tokenStatus.isExpired ? "destructive" : "outline"}
                    onClick={handleRenewToken}
                    disabled={renewingToken}
                    className="mt-2"
                  >
                    {renewingToken ? (
                      <><Loader2 className="w-3 h-3 mr-1 animate-spin" /> Renovando...</>
                    ) : (
                      <><RefreshCw className="w-3 h-3 mr-1" /> Tentar Renovar Token</>
                    )}
                  </Button>
                )}
              </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setMetaData({ ...metaData, pageId: e.target.value })}
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                value={metaData.instagramId}
                onChange={(e) => setMetaData({ ...metaData, instagramId: e.target.value })}
              />
            </div>
          </div>
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => setMetaData({ ...metaData, webhookUrl: e.target.value })}
            />
          </div>
          <div className="flex gap-2">
            <Button 
              onClick={handleTestMetaConnection} 
              disabled={testingMeta || !metaData.pageId}
              variant="outline"
              className="flex-1"
            >
              {testingMeta ? (
            ...
            <Button 
              ...
              disabled={checkingPermissions || !metaData.pageId}
              variant="outline"
            >
              {checkingPermissions ? (
                ...
                <div className="flex items-center gap-2">
                  {pageName && (
                    <Badge variant="outline" className="text-xs">
                      <Facebook as FacebookIcon className="w-3 h-3 mr-1" />
                      {pageName}
                    </Badge>
                  )}
                  {tokenType && (
                    <Badge variant={tokenType === 'user_token' ? 'default' : 'secondary'} className="text-xs">
                ...
                <div className={`flex items-start gap-2 p-3 rounded-lg text-xs border ${
                  ...
                  <div>
                    <p className="font-medium">
                      {identityTest.working 
                        ? '✅ Identificação de autores funcionando!' 
                        : '❌ Autores dos comentários NÃO estão sendo identificados'}
                    </p>
                    <p className="mt-0.5">
                      {identityTest.working
                        ? 'O token consegue ler nome e ID dos comentaristas.'
                ...
                {permissions.map((perm) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {isRequired && !perm.granted && (
                        <Badge variant="destructive" className="text-[10px] px-1 py-0 ml-auto">!</Badge>
                      )}
                    </div>
                  );
                })}
              ...
              {permissions.some(p => !p.granted && REQUIRED_PERMISSIONS.find(r => r.name === p.name)?.required) && (
                <div className="flex items-start gap-2 p-3 bg-destructive/10 border border-destructive/20 rounded-lg text-xs text-destructive">
                  ...
                  <div className="space-y-1">
                    ...
                    <p>
                      Isso impede a identificação de autores nos comentários. 
                      Para corrigir: acesse o <strong>Meta for Developers</strong>, edite seu App, vá em <strong>Permissões</strong> e solicite as permissões marcadas com "!".
                    </p>
                    <p className="text-muted-foreground">
                      Depois de aprovar, gere um novo token e salve aqui.
                    </p>
                  </div>
                </div>
              )}
              
              {/* Instagram as InstagramIcon-specific warning */}

...
import {
  Shield, LogOut, CheckCircle2, Loader2, ExternalLink, Facebook as FacebookIcon as Facebook as FacebookIcon,
  Instagram as InstagramIcon as Instagram as InstagramIcon, CalendarCheck, UserPlus, Eye, EyeOff, Edit2, Save, X,
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      if (editFacebook.trim() && !newFb) {
        toast.error("Não consegui identificar o usuário do Facebook as FacebookIcon. Cole a URL do seu perfil.");
        setSavingProfile(false);
        return;
      }
      if (editInstagram.trim() && !newIg) {
        toast.error("Não consegui identificar o usuário do Instagram as InstagramIcon. Cole a URL do seu perfil.");
        setSavingProfile(false);
        return;
      }
                    ...
                    <div className="flex flex-wrap gap-2 pt-1">
                      {account?.facebook_username && (
                        <Badge variant="outline" className="text-xs gap-1">
                          <Facebook as FacebookIcon className="w-3 h-3 text-blue-600" />
                          @{account.facebook_username}
                        </Badge>
                      )}
                      {account?.instagram_username && (
                        <Badge variant="outline" className="text-xs gap-1">
                          <Instagram as InstagramIcon className="w-3 h-3 text-pink-500" />
                          @{account.instagram_username}
                        </Badge>
                      )}
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-1">
                      <Label className="text-xs flex items-center gap-1">
                        <Facebook as FacebookIcon className="w-3 h-3 text-blue-600" /> Facebook as FacebookIcon (username)
                      </Label>
                    ...
                    <div className="space-y-1">
                      <Label className="text-xs flex items-center gap-1">
                        <Instagram as InstagramIcon className="w-3 h-3 text-pink-500" /> Instagram as InstagramIcon (username)
                      </Label>

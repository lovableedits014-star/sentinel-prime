...
import {
  exportEleicaoPdf,
  exportEleicaoCsv,
  exportEleicaoPdfRaiz,
  exportEleicaoCsvRaiz,
  type ExportPessoa,
} from "@/lib/eleicao-export-pdf";
import ExportEleicaoDialog, { type ExportConfig } from "@/components/eleicao/ExportEleicaoDialog";
import {
  exportarCsvConfiguravel,
  exportarPdfConfiguravel,
  exportarZipPorCoordenador,
} from "@/lib/eleicao-export-configuravel";
...
import {
  getEleicaoSituacao,
  isEleicaoContratado,
  isEleicaoContratadoRemunerado,
  isEleicaoSemContrato,
  isEleicaoVoluntario,
  isEleicaoVoluntarioContratado,
...
async function sendCoordBoasVindas(pessoaId: string) {
  try {
    ...
    if (!resp.ok || data?.error) {
      toast.warning("Não foi possível enviar a mensagem de boas-vindas", {
...
async function sendCaboBoasVindas(pessoaId: string) {
  try {
    ...
    const data = await resp.json().catch(() => ({}));
    if (!resp.ok || data?.error) {
      toast.warning("Não foi possível enviar a mensagem ao cabo eleitoral", {
        description: data?.error,
      });
      return;
    }
...
async function gerarContratosLote(pessoas: Pessoa[], clientId: string, zipName: string) {
  ...
  if (elegiveis.length === 0) {
    toast.error(
      "Nenhuma pessoa do time com valor definido. Defina os valores em 'Pendentes de valor'.",
    );
    return;
  }
...
export default function Eleicao() {
  ...
  async function save() {
    ...
    if (form.escopo === "interior" && !form.cidade.trim()) {
      toast.error("Cidade é obrigatória para Interior");
      return;
    }
    ...
    if (
      ...
      toast.error("Para enviar acesso, informe e-mail e senha com no mínimo 6 caracteres");
      return;
    }
    const rua = form.rua.trim();
    const numero = form.numero.trim();
    const bairro = form.bairro.trim();
    const enderecoConcat = [rua ? `${rua}${numero ? ", " + numero : ""}` : "", bairro]
      .filter(Boolean)
      .join(" - ");
    ...
    const payload: any = {
      ...
      cidade:
        form.escopo === "interior" ? form.cidade.trim() : form.cidade.trim() || "Campo Grande",
      nome: form.nome.trim(),
      ...
      email:
        form.tipo === "coordenador" && form.email.trim() ? form.email.trim().toLowerCase() : null,
      valor_contratacao: valorContratacao,
      vigencia_inicio: form.vigencia_inicio || null,
      vigencia_fim: form.vigencia_fim || null,
      status_contratacao: statusContratacao,
      confirmado_em:
        statusContratacao === "confirmado"
          ? editing?.confirmado_em || new Date().toISOString()
          : null,
      participou_reuniao: form.participou_reuniao,
      reuniao_em:
        form.participou_reuniao && !editing?.participou_reuniao
          ? new Date().toISOString()
          : editing?.reuniao_em || null,
    ...
    if (desvincularFilhosDe) {
      ...
      if (errDesv) {
        toast.error(
          "Cadastro atualizado, mas falhou ao desvincular subordinados: " + errDesv.message,
        );
      } else {
    ...
    if (!editing && savedPessoa) {
      ...
      let skip = false;
      try {
        skip = sessionStorage.getItem("eleicao:skip-pos-cadastro") === "1";
      } catch {}
      if (skip) {
        if (form.tipo === "lider") {
  ...
  async function toggleArchive(p: Pessoa) {
    if (!p.arquivado_em) {
      if (!isEleicaoSemContrato(p)) {
        toast.error(
          p.is_voluntario
            ? "Voluntários ativos não podem ser arquivados."
            : "Somente pessoas sem contrato podem ser arquivadas.",
        );
        return;
      }
      ...
      if (filhosAtivos.length > 0) {
        toast.error(
          `Transfira ou desvincule os ${filhosAtivos.length} subordinado(s) ativos antes de arquivar.`,
        );
        return;
      }
  ...
  async function togglePermissaoCadastro(
    p: Pessoa,
    field: "pode_cadastrar_lider" | "pode_cadastrar_cabo",
  ) {
    ...
    toast.success(
      novoValor
        ? `${p.nome} pode cadastrar ${label}`
        : `${p.nome} bloqueado para cadastrar ${label}`,
    );
  }
  ...
  async function saveCred() {
    ...
    if (!credEmail.trim() || !isValidEmail(credEmail) || credPassword.length < 6) {
      toast.error("Email e senha (mín. 6) obrigatórios");
      return;
    }
    setCredLoading(true);
    try {
      const ok = await sendCredentials(credPessoa, "whatsapp", {
  ...
  async function sendCredentials(
    ...
    try {
      ...
      if (data.sent)
        toast.success(
          senhaMantida
            ? "Acesso enviado por WhatsApp (senha atual mantida)"
            : "Credenciais enviadas por WhatsApp!",
        );
      else if (data.warning) toast.warning(data.warning);
      else
        toast.success(
          senhaMantida ? "Link gerado (senha atual mantida)" : "Credenciais geradas! Copie abaixo.",
        );
  ...
  const stats = useMemo(() => {
    ...
    const semValor = remunerados.filter(
      (p) => !p.valor_contratacao || p.valor_contratacao === 0,
    ).length;
  ...
  async function handleExport(cfg: ExportConfig) {
    ...
    if (cfg.coordenadorId) {
      ...
      const lideresDoCoord = new Set(
        pessoas.filter((p) => p.parent_id === coord.id).map((p) => p.id),
      );
      lista = base.filter(
        (p) =>
          p.id === coord.id ||
    ...
    if (cfg.zipPorCoordenador) {
      ...
      try {
        const qtd = await exportarZipPorCoordenador(
          listaTipada.map(toExportPessoa),
          cfg.campos,
          escopo === "campo_grande" ? "Campo Grande" : "Interior",
        );
        toast.dismiss(toastId);
        toast.success(`ZIP gerado com ${qtd} PDF(s).`);
      } catch (error: any) {
        toast.dismiss(toastId);
        toast.error(error?.message || "Não foi possível gerar o ZIP.");
      }
      return;
    }
    ...
    const baseFiltros = (): { label: string; value: string }[] => {
      ...
      if (cfg.regiao) {
        ...
        const valor =
          escopo === "interior"
            ? cfg.regiao
            : REGIOES.find((r) => r.value === cfg.regiao)?.label || cfg.regiao;
        f.push({ label, value: valor });
      }
      f.push({
        label: "Tipos",
        value: cfg.tipos
          .map((t) => (t === "coordenador" ? "Coord" : t === "lider" ? "Líder" : "Cabo"))
          .join(", "),
      });
      if (cfg.coordenadorId) {
      ...
      if (volMode === "excluir")
        f.push({ label: "Voluntários", value: "Excluídos (só remunerados)" });
      f.push({
        label: "Situação",
        value:
    ...
    const rodarExport = (
      ...
      if (cfg.formato === "csv") {
        exportarCsvConfiguravel(
          configuraveis,
          cfg.campos,
          `cadastros-eleicao-${fileNameSuffix || escopoLabel}`,
        );
      ...
      if (cfg.formato === "pdf" || cfg.formato === "print") {
        exportarPdfConfiguravel(
          configuraveis,
          cfg.campos,
          cfg.modo === "raiz" ? "Estrutura por Coordenador" : "Cadastros da Eleição",
          escopoLabel,
          cfg.formato === "print",
          cfg.modo === "raiz",
        );
    ...
    if (cfg.porParceiro && !cfg.parceiroId) {
      ...
      toast.success(
        `${totalArquivos} arquivo(s) gerado(s) · ${totalRegistros} registro(s) no total`,
      );
      return;
    }
    ...
    toast.success(
      cfg.formato === "print"
        ? "PDF aberto para impressão em nova aba."
        : `${cfg.formato.toUpperCase()} exportado (${qtd} registros)`,
    );
  }
  ...
  const formularioData = (lider: Pessoa): LiderFormularioCabos => {
    const coordenador = lider.parent_id
      ? pessoas.find((p) => p.id === lider.parent_id && p.tipo === "coordenador")
      : null;
    return {
      id: lider.id,
      nome: lider.nome,
      ...
      regiao: lider.regiao
        ? REGIOES.find((r) => r.value === lider.regiao)?.label || lider.regiao
        : null,
      cidade: lider.cidade,
  ...
  const baixarFormularioCabos = async (pessoa: Pessoa) => {
    const lideres =
      pessoa.tipo === "coordenador"
        ? pessoas.filter((p) => p.tipo === "lider" && p.parent_id === pessoa.id && !p.arquivado_em)
        : pessoa.tipo === "lider" && !pessoa.arquivado_em
          ? [pessoa]
          : [];
    if (!lideres.length)
      return toast.error("Nenhum líder ativo encontrado para gerar o formulário.");
    try {
      ...
      toast.success(
        lideres.length === 1
          ? "Formulário dos cabos gerado."
          : `${lideres.length} formulários gerados.`,
      );
    } catch (error) {
  ...
  const baixarTodosFormulariosCabos = async () => {
    const lideres = pessoas.filter(
      (p) => p.tipo === "lider" && !p.arquivado_em && p.escopo === escopo,
    );
    if (!lideres.length) return toast.error("Nenhum líder ativo neste escopo.");
    try {
      await gerarFormularioCabosPdf(
        lideres.map(formularioData),
        `Formularios de Cabos - ${escopo}`,
      );
  ...
  return (
    <EleicaoActionsContext.Provider
      value={{
        onTogglePermissao: togglePermissaoCadastro,
    ...
    >
      <EleicaoSearchContext.Provider value={searchCtxValue}>
        <div className="container mx-auto p-4 md:p-6 max-w-7xl">
          ...
          {view === "reunioes" ? (
            clientId ? (
              <ReunioesPanel clientId={clientId} />
            ) : null
          ) : view === "custos" ? (
            ...
            <FunnelManagement
              pessoas={
                pessoas
                  .filter((p) => !p.arquivado_em && !p.is_voluntario)
                  .map((p) => ({
                    ...p,
                    status_contratacao:
                      Number(p.valor_contratacao || 0) > 0 ? "confirmado" : "pendente",
                  })) as any
              }
              onEdit={openEdit}
            ...
            clientId ? (
              <PendentesValorPanel clientId={clientId} onChanged={load} />
            ) : null
          ) : view === "indicacoes" ? (
            clientId ? (
              <IndicacoesPanel clientId={clientId} />
            ) : null
          ) : view === "relatorio_contratados" ? (
            clientId ? (
              <ContratadosCumprimentoReport clientId={clientId} />
            ) : null
            ...
            clientId ? (
              <DistribuicaoContatosTab clientId={clientId} />
            ) : null
          ) : view === "dobradinhas" ? (
            clientId ? (
              <DobradinhasManagerPanel
                clientId={clientId}
            ...
            clientId ? (
              <EleicaoConfigPanel clientId={clientId} />
            ) : null
          ) : (
            <Tabs
              value={escopoTab}
            ...
            >
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-8 gap-2 mb-4">
                <KpiCard
                  ...
                  icon={Network}
                  tone="neutral"
                  hint={`${stats.arquivados} arquivado(s)${escopoTab === "geral" ? " · todos os escopos" : ""}`}
                />
                <KpiCard
                  ...
                  icon={CheckCircle2}
                  tone="blue"
                  hint={`+ ${stats.voluntariosContratados} contratos simbólicos`}
                />
                <KpiCard
                  label="Coordenadores"
                  value={stats.coord}
                  icon={Crown}
                  tone="red"
                  hint={`${stats.coordContratados} com contrato`}
                />
                <KpiCard
                  label="Líderes"
                  value={stats.lider}
                  icon={UserCheck}
                  tone="blue"
                  hint={`${stats.liderContratados} com contrato${escopoTab === "geral" ? " · Campo Grande + Interior" : ""}`}
                />
                <KpiCard
                  ...
                  icon={Users}
                  tone="green"
                  hint={`${stats.caboContratados} com contrato`}
                />
                <KpiCard
                  label="Sem contrato"
                  value={stats.semContrato}
                  icon={AlertCircle}
                  tone="amber"
                  hint="aguardando definição"
                />
                <KpiCard
                  label="Voluntários"
                  ...
                  icon={Heart}
                  tone="emerald"
                  hint="sem remuneração"
                />
                <KpiCard
                  label="Investimento"
                  value={fmtBRL(stats.valorTotal)}
                  icon={DollarSign}
                  tone="emerald"
                  hint={`contratos remunerados${escopoTab === "geral" ? " · Campo Grande + Interior" : ""}`}
                  small
                />
              </div>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <Select value={statusFilter} onValueChange={(v) => setStatusFilter(v as any)}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="voluntarios_contratados">
                        💚 Voluntários contratados
                      </SelectItem>
                ...
                <div className="flex items-center gap-2 mb-3 text-xs text-muted-foreground">
                  <span>
                    ...
                    <strong className="text-foreground">
                      {search ? matchedIds.size : escopoList.length}
                    </strong>{" "}
                    resultados
                  </span>
                  <button
              ...
              {escopoTab === "campo_grande" && (
                <div className="flex flex-wrap gap-1.5 mb-4">
                  ...
                  {(() => {
                    const avulsosCount = pessoas.filter(
                      (p) =>
                        p.escopo === escopo &&
                        p.tipo === "lider" &&
                        !p.parent_id &&
                        !p.is_voluntario &&
                        (!search || matchedIds.has(p.id)),
                    ).length;
                    const active = statusFilter === "avulsos";
                    return (
                      <button
                        onClick={() => {
                          setStatusFilter(active ? "todos" : "avulsos");
                          setRegiaoFilter("all");
                        }}
                        className={cn(
                          "px-2.5 py-1 rounded-md text-xs font-medium border transition-colors",
                          active
                            ...
                            : "bg-amber-50 dark:bg-amber-900/20 hover:bg-amber-100 border-amber-200 dark:border-amber-800 text-amber-700 dark:text-amber-300",
                  ...
                  {(() => {
                    const voluntariosCount = pessoas.filter(
                      (p) =>
                        p.escopo === escopo &&
                        !!p.is_voluntario &&
                        (!search || matchedIds.has(p.id)),
                    ...
                    return (
                      <button
                        onClick={() => {
                          setStatusFilter(active ? "todos" : "voluntarios");
                          setRegiaoFilter("all");
                        }}
                        className={cn(
                          "px-2.5 py-1 rounded-md text-xs font-medium border transition-colors",
                          active
                            ...
                            : "bg-emerald-50 dark:bg-emerald-900/20 hover:bg-emerald-100 border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-300",
                  ...
                  {REGIOES.map((r) => {
                    const count = search
                      ? escopoList.filter((p) => p.regiao === r.value && matchedIds.has(p.id))
                          .length
              ...
              <TabsContent value="campo_grande" className="space-y-2 mt-0">
                {loading ? (
                  ...
                  <ListaPlana
                    pessoas={
                      regiaoFilter === "all"
                        ? escopoList
                        : escopoList.filter((p) => p.regiao === regiaoFilter)
                    }
                    sortBy={sortBy}
                    onEdit={openEdit}
                    onDelete={remove}
                ...
                {!loading && escopoList.length === 0 && (
                  <Card className="py-12 text-center text-muted-foreground border-dashed">
                    <Crown className="w-10 h-10 mx-auto mb-2 opacity-30" />
                    <p className="text-sm">Nenhum cadastro encontrado</p>
                    <Button variant="link" onClick={() => openNew({ escopo: "campo_grande" })}>
                      Cadastrar primeiro
                    </Button>
                  </Card>
                )}
              ...
              <TabsContent value="interior" className="space-y-2 mt-0">
                {loading ? (
                  ...
                  <Card className="py-12 text-center text-muted-foreground border-dashed">
                    ...
                    <Button variant="link" onClick={() => openNew({ escopo: "interior" })}>
              ...
              {cadastroCaboResponsavel ? (
                <div className="space-y-4 px-6 py-3 overflow-y-auto flex-1 min-h-0">
                  ...
                  <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/5 p-3">
                    ...
                    <div className="relative mt-1.5">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setForm((f) => ({
                            ...f,
                            valor_contratacao: e.target.value.replace(/[^\d,.]/g, ""),
                          }))
                        }
                        placeholder="0,00"
                        inputMode="decimal"
                    ...
                    <p className="mt-1 text-[11px] text-muted-foreground">
                      Pagamento único contabilizado nos custos da campanha.
                    </p>
                  </div>
                </div>
              ) : (
                <div className="space-y-3 px-6 py-2 overflow-y-auto flex-1 min-h-0">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <Select
                        value={form.tipo}
                        onValueChange={(v) =>
                          setForm((f) => ({ ...f, tipo: v as Tipo, parent_id: "" }))
                        }
                      >
                        <SelectTrigger>
                          <SelectValue />
                    ...
                    <div>
                      ...
                      <Select
                        value={form.escopo}
                        onValueChange={(v) =>
                          setForm((f) => ({ ...f, escopo: v as Escopo, parent_id: "" }))
                        }
                      >
                        <SelectTrigger>
                          <SelectValue />
                  ...
                  {form.escopo === "campo_grande" ? (
                    <div>
                      ...
                      <Select
                        value={form.regiao}
                        onValueChange={(v) =>
                          setForm((f) => ({ ...f, regiao: v as Regiao, parent_id: "" }))
                        }
                      >
                        <SelectTrigger>
                          <SelectValue />
                    ...
                    <div>
                      ...
                      <Input
                        value={form.cidade}
                        onChange={(e) =>
                          setForm((f) => ({ ...f, cidade: e.target.value, parent_id: "" }))
                        }
                      ...
                      <div>
                        <Label className="flex justify-between items-center">
                          <span>
                            {form.tipo === "lider"
                              ? "Coordenador Responsável"
                              : "Vinculado a (Líder ou Coordenador)"}
                          </span>
                          {form.tipo === "lider" && (
                            <button
                              type="button"
                        ...
                        >
                          ...
                          <SelectContent>
                            ...
                            {(() => {
                              ...
                              const escopoValue =
                                form.escopo === "interior" ? form.cidade : form.regiao;
                              ...
                              if (potential.length === 0)
                                return (
                                  <SelectItem value="no-options" disabled>
                                    Nenhum superior disponível nesta localidade
                                  </SelectItem>
                                );
                                  ...
                                  <SelectItem key={p.id} value={p.id}>
                                    <span className="flex items-center gap-2">
                                      <Badge
                                        variant="outline"
                                        className={cn(
                                          "text-[9px] h-4 px-1 leading-none uppercase",
                                          TIPO_META[p.tipo].color,
                                        )}
                                      >
                        ...
                        {!form.parent_id && (
                          <p className="text-[10px] text-muted-foreground italic mt-1 flex items-center gap-1">
                            ...
                            Este {TIPO_META[form.tipo].label} será cadastrado como Avulso (sem
                            superior).
                          </p>
                        )}
                      </div>
                    ...
                    <div className="rounded-md border border-border bg-muted/20 p-2 flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-[9px] text-muted-foreground">
                          Ao definir o valor, passa para Contratados.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2 rounded-md border border-blue-500/25 bg-blue-500/5 p-3">
                      <div>
                        ...
                        <p className="text-[10px] text-muted-foreground">
                          Desmarque a rede que esta pessoa não possui. No portal aparecerão somente
                          as redes selecionadas.
                        </p>
                      </div>
                      <div className="grid grid-cols-2 gap-2">
                        <label className="flex cursor-pointer items-center gap-2 rounded-md border bg-background p-2 text-sm">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setForm((f) => ({ ...f, missao_facebook_ativo: !!checked }))
                            }
                          />
                          Facebook
                        </label>
                        <label className="flex cursor-pointer items-center gap-2 rounded-md border bg-background p-2 text-sm">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setForm((f) => ({ ...f, missao_instagram_ativo: !!checked }))
                            }
                          />
                          Instagram
                        </label>
                    ...
                    <div className="space-y-3 rounded-md border border-border bg-muted/20 p-3">
                          ...
                          <div>
                            <div className="flex items-center justify-between gap-2">
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  setForm((f) => ({ ...f, password: genLocalPassword() }))
                                }
                              >
                                Gerar
                              </Button>
                      ...
                      {!editing && (
                        <label className="flex items-center gap-2 text-sm font-medium">
                          <Checkbox
                            checked={form.send_access}
                            onCheckedChange={(checked) =>
                              setForm((f) => ({ ...f, send_access: !!checked }))
                            }
                          />
                  ...
                  <div className="grid grid-cols-[1fr_100px] gap-2">
                    <div>
                      ...
                      <Input
                        value={form.rua}
                        onChange={(e) => setForm((f) => ({ ...f, rua: e.target.value }))}
                        placeholder="Av. Afonso Pena (opcional)"
                      />
                    </div>
                    <div>
                      <Label>Nº</Label>
                  ...
                  <div className="rounded-md border p-3 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setForm((f) => ({ ...f, cep: onlyDigits(e.target.value).slice(0, 8) }))
                          }
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setForm((f) => ({ ...f, rg_orgao_expedidor: e.target.value }))
                          }
                    ...
                    <p className="text-xs text-muted-foreground">
                      Vigência, dados bancários e a data do contrato ficam em branco no documento
                  ...
                  {(() => {
                    const isRaiz =
                      form.tipo === "coordenador" || (form.tipo === "lider" && form.liderAvulso);
                    if (!isRaiz) {
                      ...
                      return (
                        <div className="rounded-md border bg-muted/30 p-3 text-xs space-y-1">
                          <div className="flex items-center gap-1.5 font-medium text-muted-foreground">
                            <Handshake className="w-3.5 h-3.5" />
                            Dobradinha herdada do time de{" "}
                            <strong className="text-foreground">{parent.nome}</strong>
                          </div>
                          {parc ? (
                            <div className="inline-flex items-center gap-1.5">
                              <span
                                className="w-2 h-2 rounded-full"
                                style={{ backgroundColor: parc.cor }}
                              />
                              <span className="font-medium">{parc.nome}</span>
                              <span className="text-muted-foreground">
                                · {parent.rateio_estadual ?? 100}% estadual /{" "}
                                {parent.rateio_parceiro ?? 0}% federal
                              </span>
                            </div>
                          ) : (
                      ...
                      PARCEIROS.length > 0 && (
                        <div className="rounded-md border border-primary/30 bg-primary/5 p-3 space-y-3">
                          ...
                          <div>
                            ...
                            >
                              <SelectTrigger>
                                <SelectValue placeholder="Sem dobradinha — só estadual" />
                              </SelectTrigger>
                              <SelectContent>
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5">
                                {[
                                  ...
                                  const active =
                                    form.rateio_estadual === opt.e &&
                                    form.rateio_parceiro === opt.p;
                                  return (
                                    <Button
                                      key={opt.label}
                                      type="button"
                                      ...
                                      onClick={() =>
                                        setForm((f) => ({
                                          ...f,
                                          rateio_estadual: opt.e,
                                          rateio_parceiro: opt.p,
                                        }))
                                      }
                                    >
                                      {opt.label}
                              ...
                              <p className="text-[11px] text-muted-foreground">
                                A soma sempre é 100%. Ao definir um lado, o outro ajusta
                                automaticamente.
                              </p>
                            </div>
                          )}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <p className="col-span-2 text-[11px] text-muted-foreground -mt-1">
                      Usado na Cláusula Segunda do contrato. Se ficar vazio, sai em branco para
                      preencher à mão.
                    </p>
                  </div>
          ...
          <Dialog open={credOpen} onOpenChange={setCredOpen}>
            <DialogContent className="max-w-sm w-[calc(100vw-2rem)] max-h-[calc(100dvh-2rem)] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>Acesso ao portal — {credPessoa?.nome}</DialogTitle>
              </DialogHeader>
              <p className="text-xs text-muted-foreground">
                Defina email e senha para salvar o acesso e enviar pelo WhatsApp conectado.
              </p>
              ...
              <DialogFooter>
                <Button variant="ghost" onClick={() => setCredOpen(false)}>
                  Cancelar
                </Button>
                <Button onClick={saveCred} disabled={credLoading}>
                  {credLoading ? "Enviando…" : "Salvar e enviar"}
                </Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
                ...
                <div className="space-y-3">
                    ...
                    <div className="flex flex-col gap-0.5">
                      ...
                      {credResult.password ? (
                        ...
                        <span className="text-xs italic text-muted-foreground">
                          Senha atual mantida (não foi alterada)
                        </span>
                      )}
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <Button
                      ...
                      className="flex-1"
                      onClick={() => {
                        navigator.clipboard.writeText(credResult.message);
                    ...
                    {!credResult.sent && (
                      <Button
                        className="flex-1"
                        disabled={sendingId === credResult.pessoa.id}
                        onClick={() => sendCredentials(credResult.pessoa, "whatsapp")}
                      >
                        <Send className="w-4 h-4 mr-2" />
                        {sendingId === credResult.pessoa.id ? "Enviando..." : "Enviar via WhatsApp"}
                      </Button>
                    )}
                  </div>
                  {credResult.password && (
                    <p className="text-[11px] text-muted-foreground">
                      Salve esta senha — ela só aparece aqui neste momento.
                    </p>
                  )}
                </div>
              )}
          ...
          <PosCadastroEnvioDialog
            ...
            onOpenChange={(o) => {
              setPosCadastroOpen(o);
              if (!o) setPosCadastroPessoa(null);
            }}
    ...
    <Card className="overflow-hidden border-border/60 shadow-sm">
      ...
      >
        ...
        <div className="flex items-center gap-1 text-[11px] text-muted-foreground shrink-0">
          {coords.length > 0 && (
            <span
              className="px-1.5 py-0.5 rounded bg-red-500/10 text-red-600 font-medium"
              title="Coordenadores"
            >
              {coords.length}
          ...
          {cabos.length > 0 && (
            <span
              className="px-1.5 py-0.5 rounded bg-green-500/10 text-green-600 font-medium"
              title="Cabos"
            >
              {cabos.length}
        ...
        {hasContent && (
          <Button
            ...
            onClick={(e) => {
              e.stopPropagation();
              gerarContratosLote(pessoas, pessoas[0].client_id, `Contratos - ${title}`);
            }}
              ...
              <div className="flex items-center justify-between gap-2 mb-1.5">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <p className="text-[10px] uppercase tracking-wide font-semibold text-amber-700 dark:text-amber-400 truncate">
                    ...
                    {(() => {
                      ...
                      return (
                        <span className="font-normal text-muted-foreground normal-case ml-1">
                          · {fmtBRL(tot)}
                          {sv > 0 ? ` · ${sv} sem valor` : ""}
                        </span>
                      );
                ...
                <Button
                  ...
                  onClick={(e) => {
                    ...
                    gerarContratosLote(
                      lideresOrfaos,
                      lideresOrfaos[0].client_id,
                      `Líderes avulsos - ${title}`,
                    );
                  }}
...
function CoordBlock({
  ...
  const cabosLid = lideres.flatMap((l) =>
    all.filter((p) => p.tipo === "cabo" && p.parent_id === l.id),
  );
            ...
            ? {
                ...
                onClick: () => {
                  const local =
                    coord.cidade ||
                    (coord.regiao
                      ? coord.regiao.charAt(0).toUpperCase() + coord.regiao.slice(1)
                      : "");
                  const zipName = local
...
function FavoritoToggle({ pessoa }: { pessoa: Pessoa }) {
  ...
  async function toggle(e: React.MouseEvent) {
    ...
    if (isFav) {
      if (
        !confirm(
          `Remover ${pessoa.nome} como coordenador favorito da região ${regiaoLabel}?\n\nSem favorito, novos líderes notificarão o coordenador mais antigo da região.`,
        )
      )
        return;
    } else {
      if (
        !confirm(
          `Definir ${pessoa.nome} como coordenador favorito da região ${regiaoLabel}?\n\nEle passará a receber as notificações de novos líderes cadastrados nesta região (substituindo o atual favorito, se houver).`,
        )
      )
        return;
    }
  ...
  return (
    ...
    >
      {onToggle ? (
        ...
        <div className="w-3.5 shrink-0 text-muted-foreground/30 text-xs font-mono leading-none">
          └
        </div>
      ) : (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 flex-wrap">
          <span className={cn("text-sm truncate", indent === 0 ? "font-semibold" : "font-medium")}>
            {p.nome}
          </span>
          {/* Badge de pré-selecionado removida conforme plano */}
          {p.participou_reuniao && (
            <Badge
              variant="outline"
              className="h-5 bg-blue-500/10 text-blue-600 border-blue-500/20 text-[9px] gap-1 px-1"
            >
              <Users className="w-2.5 h-2.5" /> Reunião
            </Badge>
          )}
          <Badge
            variant="outline"
          ...
          {p.tipo === "coordenador" && p.user_id && (
            <CheckCircle2
              ...
              aria-label="Acesso configurado"
            />
          )}
          {p.tipo === "coordenador" && p.pode_cadastrar_lider === false && (
            <Badge
              variant="outline"
              className="h-4 px-1 text-[9px] border-amber-500/40 text-amber-700 bg-amber-500/10 shrink-0 gap-0.5"
              title="Bloqueado para cadastrar líderes"
            >
              🔒 Líderes
            </Badge>
          )}
          {p.tipo === "coordenador" && p.pode_cadastrar_cabo === false && (
            <Badge
              variant="outline"
              className="h-4 px-1 text-[9px] border-amber-500/40 text-amber-700 bg-amber-500/10 shrink-0 gap-0.5"
              title="Bloqueado para cadastrar cabos"
            >
              🔒 Cabos
            </Badge>
          )}
          {situacao === "sem_contrato" ? (
            <Badge
              variant="outline"
              className="h-4 px-1 text-[9px] border-amber-500/40 text-amber-600 bg-amber-500/10 shrink-0 gap-0.5"
            >
        ...
        {parentName && (p.tipo === "lider" || p.tipo === "cabo") && (
          <div className="text-[10.5px] text-muted-foreground/90 mt-0.5 italic truncate">
            ↳ Vinculado a {parentTipo === "coordenador" ? "coordenador" : "líder"}{" "}
            <span className="font-medium text-foreground/80 not-italic">{parentName}</span>
          </div>
        )}
        {teamFinancials && (
          <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 mt-1 text-[10.5px] tabular-nums">
            <span className="text-muted-foreground">
              Coordenador{" "}
              <strong className="text-foreground">{fmtBRL(teamFinancials.valorCoordenador)}</strong>
            </span>
            <span className="text-muted-foreground">
              Líderes ({teamFinancials.lideres}){" "}
              <strong className="text-blue-700 dark:text-blue-400">
                {fmtBRL(teamFinancials.valorLideres)}
              </strong>
            </span>
            <span className="text-muted-foreground">
              Cabos ({teamFinancials.cabos}){" "}
              <strong className="text-green-700 dark:text-green-400">
                {fmtBRL(teamFinancials.valorCabos)}
              </strong>
            </span>
            <span className="rounded bg-primary/10 px-1.5 py-0.5 font-semibold text-primary">
              Total da árvore: {fmtBRL(teamFinancials.valorTotal)}
            </span>
          </div>
        )}
      </div>
      ...
      {teamCount !== undefined && (
        <Badge
          variant={teamLimit && teamCount >= teamLimit ? "destructive" : "secondary"}
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild onClick={(e) => e.stopPropagation()}>
          <Button
            size="icon"
            variant="ghost"
            className="h-7 w-7 shrink-0 opacity-50 group-hover:opacity-100 focus:opacity-100"
          >
        ...
        <DropdownMenuContent align="end" onClick={(e) => e.stopPropagation()}>
          ...
          {[
            ...
            <DropdownMenuItem
              ...
              title={
                semValor
                  ? "Defina o valor em 'Pendentes de valor' para liberar o contrato"
                  : opt.label
              }
              onClick={async () => {
                try {
                  ...
                  if (r.faltando.length > 0) {
                    toast.warning(
                      `Modelo de ${r.faltando.join(" e ")} não encontrado para ${p.tipo}. Crie em "Modelos de contrato".`,
                    );
                  } else {
                    toast.success(
                      r.gerados.length > 1
                        ? "Contrato e distrato baixados (.zip)!"
                        : "Documento gerado!",
                    );
                  }
                } catch (e: any) {
            ...
            >
              ...
              {opt.label}
              {semValor && <span className="ml-auto text-[10px] opacity-60">sem valor</span>}
            </DropdownMenuItem>
          ))}
          ...
          {p.tipo === "coordenador" && onSend && (
            <>
              ...
              <DropdownMenuItem onClick={() => sendCoordBoasVindas(p.id)}>
                ...
                Enviar boas-vindas (grupo)
              </DropdownMenuItem>
              {onTogglePermissao && (
                <>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem onClick={() => onTogglePermissao(p, "pode_cadastrar_lider")}>
                    {p.pode_cadastrar_lider === false ? "✅ Permitir" : "🚫 Bloquear"} cadastro de
                    Líderes
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => onTogglePermissao(p, "pode_cadastrar_cabo")}>
                    {p.pode_cadastrar_cabo === false ? "✅ Permitir" : "🚫 Bloquear"} cadastro de
                    Cabos
                  </DropdownMenuItem>
                </>
              )}
          ...
          {onArchive && (p.arquivado_em || situacao === "sem_contrato") && (
            <DropdownMenuItem onClick={() => onArchive(p)}>
              ...
              {p.arquivado_em ? "Restaurar cadastro" : "Arquivar cadastro"}
            </DropdownMenuItem>
          )}
          <DropdownMenuItem
...
function KpiCard({
  ...
  value: number | string;
  icon: any;
  tone: "neutral" | "red" | "blue" | "green" | "emerald" | "amber";
  small?: boolean;
  hint?: string;
}) {
  const tones: Record<string, string> = {
    ...
    green:
      "from-green-500/15 to-green-500/5 text-green-700 dark:text-green-400 border-green-500/20",
    emerald:
      "from-emerald-500/15 to-emerald-500/5 text-emerald-700 dark:text-emerald-400 border-emerald-500/20",
    amber:
  ...
  return (
    <div
      className={cn(
        "relative min-h-[86px] rounded-xl border bg-gradient-to-br p-3 overflow-hidden",
        tones[tone],
      )}
    >
      <div className="min-w-0">
        <div className="flex items-start justify-between gap-1">
          <p className="min-w-0 text-[10px] uppercase tracking-wider opacity-70 font-semibold">
            {label}
          </p>
        ...
        <p
          className={cn(
            ...
            small
              ? "whitespace-nowrap text-[clamp(0.7rem,1vw,1rem)] tracking-tight"
              : "truncate text-2xl",
          )}
          title={String(value)}
        >
        ...
        {hint && (
          <p className="mt-1 text-[9px] leading-tight opacity-65 truncate" title={hint}>
            {hint}
          </p>
        )}
      </div>
...
function ListaPlana({
  ...
  const sorted = [...pessoas].sort((a, b) => {
    ...
    if (sortBy === "tipo")
      return tipoOrder[a.tipo] - tipoOrder[b.tipo] || a.nome.localeCompare(b.nome);

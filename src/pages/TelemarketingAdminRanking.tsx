...
import { useRankingIndicadores, type RankingRow } from "@/components/telemarketing/useRankingIndicadores";
...
interface MetaCfg { meta_coordenador: number; meta_lider: number; meta_cabo: number }
...
export default function TelemarketingAdminRanking() {
  ...
  const [tipoFiltro, setTipoFiltro] = useState<"coordenador" | "lider" | "liderado" | "todos">("coordenador");
  ...
  useEffect(() => {
    ...
    void supabase.from("telemarketing_campanhas" as any)
      .select("id, nome, ativo").eq("client_id", clientId)
      .order("created_at", { ascending: false })
      .then(({ data }) => setCampanhas(((data as any[]) || []) as Campanha[]));
    void supabase.from("eleicao_indicacao_config" as any)
      .select("meta_coordenador, meta_lider, meta_cabo").eq("client_id", clientId).maybeSingle()
      .then(({ data }) => setMetaCfg((data as any) || { meta_coordenador: 30, meta_lider: 30, meta_cabo: 5 }));
  }, [clientId]);
  ...
  const inativos = useMemo(() => {
    ...
    return rows.filter(r => (r.pessoa_tipo === "coordenador" || (universo === "contratados" && r.pessoa_tipo === "lider"))
    ...
    <div className="p-4 md:p-6">
      ...
      <div className="mb-4">
        ...
        <p className="text-sm text-muted-foreground">
          Quem traz mais resultado pra campanha. Clique numa linha para ver os indicados dela e o que cada um respondeu no telemarketing.
        </p>
          ...
          <div className="flex flex-col sm:flex-row sm:items-end gap-3">
            ...
            <div>
              ...
              <Tabs value={periodo} onValueChange={setPeriodo} className="mt-1">
                <TabsList>
                  {PERIODOS.map(p => <TabsTrigger key={p.id} value={p.id}>{p.label}</TabsTrigger>)}
                </TabsList>
              </Tabs>
            </div>
            <div className="ml-auto">
              <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
                <CollapsibleTrigger asChild>
                  <Button variant="ghost" size="sm">{advancedOpen ? "Ocultar" : "Filtros avançados"}</Button>
                </CollapsibleTrigger>
              </Collapsible>
            </div>
          </div>
      ...
      {inativos.length > 0 && (
        <div className="mb-4 flex items-center gap-2 text-sm bg-orange-50 dark:bg-orange-950/30 border border-orange-200 dark:border-orange-900 rounded-md px-3 py-2">
          ...
          <span><strong>{inativos.length}</strong> {inativos.length === 1 ? "líder/coordenador" : "líderes/coordenadores"} sem atividade nos últimos 7 dias.</span>
        </div>
      )}

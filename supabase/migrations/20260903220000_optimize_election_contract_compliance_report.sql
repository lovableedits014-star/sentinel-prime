...
BEGIN
  ...
  WITH RECURSIVE contratados AS MATERIALIZED (
  ...
  ), checkins_by_person AS MATERIALIZED (
    SELECT mc.mission_id,coalesce(mc.pessoa_id,mp.pessoa_id) pessoa_id,
      min(mc.primeiro_acesso_em) primeiro_acesso_em,max(mc.concluido_em) concluido_em
    ...
    WHERE mc.client_id=p_client_id
      AND coalesce(mc.pessoa_id,mp.pessoa_id) IS NOT NULL
      AND coalesce(m.publicado_em,m.created_at) >= p_data_inicio::timestamp AT TIME ZONE 'America/Campo_Grande'
      AND coalesce(m.publicado_em,m.created_at) < (p_data_fim+1)::timestamp AT TIME ZONE 'America/Campo_Grande'
    GROUP BY mc.mission_id,coalesce(mc.pessoa_id,mp.pessoa_id)
  ), checkins_by_phone AS MATERIALIZED (
    SELECT mc.mission_id,public.mission_phone_key(mp.phone_e164) phone_key,
      min(mc.primeiro_acesso_em) primeiro_acesso_em,max(mc.concluido_em) concluido_em
    FROM public.mission_checkins mc
    JOIN public.mission_participants mp ON mp.id=mc.participant_id
    JOIN public.portal_missions m ON m.id=mc.mission_id AND m.client_id=mc.client_id
    WHERE mc.client_id=p_client_id
      ...
      AND coalesce(m.publicado_em,m.created_at) >= p_data_inicio::timestamp AT TIME ZONE 'America/Campo_Grande'
      AND coalesce(m.publicado_em,m.created_at) < (p_data_fim+1)::timestamp AT TIME ZONE 'America/Campo_Grande'
    GROUP BY mc.mission_id,public.mission_phone_key(mp.phone_e164)
    ...
    SELECT c.id pessoa_id,o.mission_id,m.title AS titulo,
      ...
      CASE WHEN o.status='cumprida' OR o.cumprida_em IS NOT NULL
          OR bp.concluido_em IS NOT NULL OR bf.concluido_em IS NOT NULL THEN 'cumpriu'
        WHEN bp.primeiro_acesso_em IS NOT NULL OR bf.primeiro_acesso_em IS NOT NULL
          THEN 'abriu' ELSE 'nao_abriu' END status,
      coalesce(o.cumprida_em,bp.concluido_em,bf.concluido_em) cumprida_em,
      coalesce(bp.primeiro_acesso_em,bf.primeiro_acesso_em) primeiro_acesso_em
    FROM contratados c
    ...
    LEFT JOIN checkins_by_person bp ON bp.mission_id=o.mission_id AND bp.pessoa_id=c.id
    LEFT JOIN checkins_by_phone bf ON bf.mission_id=o.mission_id
      AND bf.phone_key=public.mission_phone_key(c.telefone)

...
CREATE INDEX IF NOT EXISTS idx_eleicao_indicados_indicador
  ON public.eleicao_indicados (indicador_id, ultima_ligacao_em);
CREATE INDEX IF NOT EXISTS idx_eleicao_pessoas_parent
  ON public.eleicao_pessoas (client_id, parent_id);
CREATE INDEX IF NOT EXISTS idx_contratado_indicados_contratado
  ON public.contratado_indicados (contratado_id, ligacao_em);
...
AS $$
BEGIN
  -- Acesso: super admin, dono do cliente ou membro ativo do time
  IF NOT (
  ...
  IF _universo = 'eleicao' THEN
    ...
    -- rollup para coordenadores: somar indicados dos líderes filhos
    pessoas AS (
    ...
    rollup AS (
      SELECT p.id AS pid,
        COALESCE((SELECT sum(d.qtd) FROM direto d
                  WHERE d.pid = p.id
                     OR d.pid IN (SELECT id FROM pessoas WHERE parent_id = p.id)), 0)::int AS qtd,
        COALESCE((SELECT sum(d.lig) FROM direto d
                  WHERE d.pid = p.id
                     OR d.pid IN (SELECT id FROM pessoas WHERE parent_id = p.id)), 0)::int AS lig,
        COALESCE((SELECT sum(d.conf) FROM direto d
                  WHERE d.pid = p.id
                     OR d.pid IN (SELECT id FROM pessoas WHERE parent_id = p.id)), 0)::int AS conf,
        COALESCE((SELECT sum(d.ind) FROM direto d
                  WHERE d.pid = p.id
                     OR d.pid IN (SELECT id FROM pessoas WHERE parent_id = p.id)), 0)::int AS ind,
        COALESCE((SELECT sum(d.rej) FROM direto d
                  WHERE d.pid = p.id
                     OR d.pid IN (SELECT id FROM pessoas WHERE parent_id = p.id)), 0)::int AS rej,
        (SELECT max(d.ult) FROM direto d
          WHERE d.pid = p.id
             OR d.pid IN (SELECT id FROM pessoas WHERE parent_id = p.id)) AS ult
      FROM pessoas p
    )
    SELECT
      ...
      -- meta: para coordenador soma quotas dos líderes filhos (default 10 cada); para líder, sua quota (default 10)
      CASE
        WHEN p.tipo = 'coordenador' THEN COALESCE(f.qt, 0) * 10
        ELSE 10
      END,
      r.ult
    FROM pessoas p
  ...
  ELSE -- universo = 'contratados'
    ...
    pessoas AS (
      SELECT c.id, c.nome, CASE WHEN c.is_lider THEN 'lider' ELSE 'liderado' END AS tipo,
             ...
             COALESCE(c.quota_indicados, 10) AS quota
    ...
    rollup AS (
      SELECT p.id AS pid,
        COALESCE((SELECT sum(d.qtd) FROM direto d
                  WHERE d.pid = p.id
                     OR (p.is_lider AND d.pid IN (SELECT id FROM pessoas WHERE parent_id = p.id))), 0)::int AS qtd,
        COALESCE((SELECT sum(d.lig) FROM direto d
                  WHERE d.pid = p.id
                     OR (p.is_lider AND d.pid IN (SELECT id FROM pessoas WHERE parent_id = p.id))), 0)::int AS lig,
        COALESCE((SELECT sum(d.conf) FROM direto d
                  WHERE d.pid = p.id
                     OR (p.is_lider AND d.pid IN (SELECT id FROM pessoas WHERE parent_id = p.id))), 0)::int AS conf,
        COALESCE((SELECT sum(d.ind) FROM direto d
                  WHERE d.pid = p.id
                     OR (p.is_lider AND d.pid IN (SELECT id FROM pessoas WHERE parent_id = p.id))), 0)::int AS ind,
        COALESCE((SELECT sum(d.rej) FROM direto d
                  WHERE d.pid = p.id
                     OR (p.is_lider AND d.pid IN (SELECT id FROM pessoas WHERE parent_id = p.id))), 0)::int AS rej,
        (SELECT max(d.ult) FROM direto d
          WHERE d.pid = p.id
             OR (p.is_lider AND d.pid IN (SELECT id FROM pessoas WHERE parent_id = p.id))) AS ult
      FROM pessoas p
    )
    SELECT
      p.id, p.nome, p.tipo, p.cidade, p.bairro,
      ...
      GREATEST(r.qtd - r.lig, 0),
      CASE WHEN r.lig > 0 THEN round((r.conf::numeric / r.lig::numeric) * 100, 1) ELSE NULL END,
      CASE WHEN p.is_lider THEN COALESCE(f.qt, 0) * 10 ELSE p.quota END,
      r.ult
    FROM pessoas p
    LEFT JOIN pessoas parent ON parent.id = p.parent_id
    LEFT JOIN filhos f ON f.pid = p.id
